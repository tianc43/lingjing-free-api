...
export function AccountDialog({ account, onClose, onSave }: { account: Account | null; onClose(): void; onSave(input: AccountInput): Promise<void> }) {
  ...
  return <dialog ref={dialogRef} className="dialog" aria-labelledby="account-dialog-title" onCancel={(event) => { event.preventDefault(); onClose(); }}><header><h2 id="account-dialog-title">{account === null ? "Create account" : `Edit ${account.name}`}</h2><button aria-label="Close dialog" className="icon-button" onClick={onClose}>×</button></header><form noValidate onSubmit={(event) => void submit(event)}>{field("name", "Account name", name, setName)}{field("priority", "Priority", priority, setPriority, "number")}{field("daily", "Daily point limit", daily, setDaily, "number")}{field("monthly", "Monthly point limit", monthly, setMonthly, "number")}{saveError && <p className="inline-error" aria-live="polite">{saveError}</p>}<footer><button type="button" className="quiet-button" onClick={onClose}>Cancel</button><button disabled={saving} type="submit">{account === null ? "Create account" : "Save account"}</button></footer></form></dialog>;
}
