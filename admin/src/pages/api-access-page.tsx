...
export function ApiAccessPage({ settings, keys, onCreate, onToggle, onRevoke }: { settings: Settings; keys: ApiKey[]; onCreate(name: string): Promise<void>; onToggle(key: ApiKey): Promise<void>; onRevoke(key: ApiKey): Promise<void> }) {
  ...
  const examples = [
    ...
    { label: "Generate image", value: [`IMAGE_MODEL="$(curl -sS "${settings.api_base_url}/models?type=image" \\`, `  -H "${authorizationHeader}" | jq -er '.data[0].id')"`, `curl -sS "${settings.api_base_url}/images/generations" \\`, `  -H "${authorizationHeader}" \\`, "  -H \"Content-Type: application/json\" \\", `  -d "$(jq -nc --arg model "$IMAGE_MODEL" '{model:$model,prompt:"A calm ink landscape"}')"`].join("\n") },
    { label: "Generate video", value: [`VIDEO_MODEL="$(curl -sS "${settings.api_base_url}/models?type=video&mode=text-to-video" \\`, `  -H "${authorizationHeader}" | jq -er '.data[0].id')"`, `curl -sS "${settings.api_base_url}/videos" \\`, `  -H "${authorizationHeader}" \\`, "  -H \"Content-Type: application/json\" \\", `  -d "$(jq -nc --arg model "$VIDEO_MODEL" '{model:$model,mode:"text-to-video",prompt:"A calm ink landscape"}')"`].join("\n") },
