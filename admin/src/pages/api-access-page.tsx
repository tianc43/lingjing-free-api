...
export function ApiAccessPage({ settings, keys, onCreate, onToggle, onRevoke }: { settings: Settings; keys: ApiKey[]; onCreate(name: string): Promise<void>; onToggle(key: ApiKey): Promise<void>; onRevoke(key: ApiKey): Promise<void> }) {
  ...
  const examples = [
    ...
    { label: "Generate video", value: [`curl "${settings.api_base_url}/videos/generations" \\`, `  -H "${authorizationHeader}" \\`, "  -H \"Content-Type: application/json\" \\", "  -d '{\"model\":\"fixture-video\",\"mode\":\"text-to-video\",\"prompt\":\"A calm ink landscape\"}'"].join("\n") },
