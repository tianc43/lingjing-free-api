...
export class AccountService {
  ...
  async describe(): Promise<AccountSnapshot> {
    ...
    const [base, spaces, member, wallet] = await Promise.all([this.dependencies.read<unknown>("/api/user/describeBaseInfo"), this.dependencies.read<unknown>("/joycreator/team/space/menu/list"), this.dependencies.read<unknown>("/joycreator/member/queryMember", { query: { pin: profile.originPin } }), this.dependencies.read<unknown>("/api/wallet/describeAccountCoupons")]);
