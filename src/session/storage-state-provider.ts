...
function cookieToPlaywright(cookie: Cookie): PlaywrightCookie {
  ...
  return {
    ...
    domain: cookie.hostOnly
      ? (cookie.domain ?? "lingjing.jdcloud.com")
      : `.${cookie.domain ?? "lingjing.jdcloud.com"}`,
    path: cookie.path ?? "/",
    expires,
...
export class StorageStateProvider implements SessionProvider {
  ...
  constructor(
    ...
    private readonly atomicWriter: AtomicWriter = atomicWritePrivateJson,
    origin: URL = new URL(SESSION_ORIGIN),
  ) {
  ...
  async load(): Promise<SessionSnapshot> {
    return this.mutex.run(async () => {
      if (this.retired) {
        if (this.jar === null || this.sourceMtimeMs === null) {
          throw new Error("Retired session snapshot is unavailable");
        }
        return {
          mode: this.mode,
          jar: this.jar,
          csrfToken: this.csrfToken,
          sourceMtimeMs: this.sourceMtimeMs,
        };
      }
  ...
  async loadProfile(): Promise<{ originPin: string }> {
    ...
    const originPin =
      typeof value === "object" && value !== null
        ? (value as { originPin?: unknown }).originPin
        : undefined;
    ...
    await this.mutex.run(async () => {
      ...
      for (const header of headers) {
        ...
        if (stored !== undefined) {
          const key = cookieKey(
            stored.key,
            stored.domain ?? url.hostname,
            stored.path ?? "/",
            stored.hostOnly ?? false,
          );
          this.changedCookies.set(key, isExpired(stored) ? null : stored);
        }
      }
      const state = storageStateFromJson(
        JSON.parse(await readFile(this.sourcePath, "utf8")) as unknown,
      );
      ...
      const cookies = state.cookies.flatMap((cookie) => {
        ...
        if (changed !== undefined || this.changedCookies.has(key)) {
          written.add(key);
          return changed === null || changed === undefined
            ? []
            : [cookieToPlaywright(changed)];
        }
        return [cookie];
      });
  ...
  describe(): {
    ...
    return {
      mode: this.mode,
      source: this.sourcePath.split(/[\\/]/u).at(-1) ?? this.sourcePath,
      sourceMtimeMs: this.sourceMtimeMs,
  ...
  private async updateCsrf(jar: CookieJar): Promise<void> {
    const csrfCookie = (await jar.getCookies(this.origin.toString())).find(
