...
1. 远端部署点击管理台目标账号的“重新登录”，在自动打开的本地灵境页面登录后，通过回填窗口提交 Cookie；本机非容器环境可使用 `npm run login`。
2. 确认账号健康、余额和对应会话目录的修改时间、所有权及权限已更新，不查看内容。
3. 远端更新成功后无需重启；调用 `/v1/session`、`/token/check` 验证。
