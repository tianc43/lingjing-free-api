...
服务会在北京时间每日 00:10 对所有已启用且会话健康的账号执行一次签到。每个账号先读取当天签到状态：已签到则跳过，未签到才提交当前活动编号，并在提交后再次读取状态确认。单账号失败不会阻塞其他账号；同一轮不会盲目重试不确定的提交，结果写入安全运行日志且不包含 Cookie、CSRF 或账号名称。PostgreSQL 多实例使用数据库 advisory lock，确保同一时刻只有一个实例执行整轮签到。
