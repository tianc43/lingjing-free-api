...
} from "../../src/session/atomic-write.js";
import {
...
describe("session providers", () => {
  it("imports Playwright cookies and mirrors csrfToken", async () => {
    ...
    expect(await snapshot.jar.getCookieString(origin)).toContain(
      "csrfToken=fixture-csrf",
    );
    expect(snapshot.csrfToken).toBe("fixture-csrf");
    expect(await provider.loadProfile()).toEqual({
      originPin: "fixture-origin-pin",
    });
  });
  ...
  it("parses a Cookie header file without exposing it in describe output", async () => {
    ...
    const snapshot = await provider.load();
    expect(await snapshot.jar.getCookieString(origin)).toContain(
      "fixture_session=fixture-session-value",
    );
  ...
  it("persists browser-state rotation but never rewrites a Cookie file", async () => {
    ...
    const profilePath = await copyFixtureToTemporaryFile(
      "session-profile.json",
    );
    const browserProvider = new StorageStateProvider(
      browserPath,
      profilePath,
      undefined,
      new URL(origin),
    ...
    expect(await readFakeCsrfFromStorageState(browserPath)).toBe(
      "fixture-rotated-csrf",
    );

    const cookiePath = await copyFixtureToTemporaryFile("cookie.txt");
    const before = await readFile(cookiePath, "utf8");
    const cookieProvider = new CookieFileProvider(
      cookiePath,
      profilePath,
      new URL(origin),
    );
  ...
  it("serializes concurrent browser-state cookie merges", async () => {
    ...
    await provider.load();
    await Promise.all([
      provider.applySetCookies(new URL(origin), [
        "cookieA=fixture-a; Path=/; Secure",
      ]),
      provider.applySetCookies(new URL(origin), [
        "cookieB=fixture-b; Path=/; Secure",
      ]),
    ]);
    const cookieHeader = await (
      await provider.load()
    ).jar.getCookieString(origin);
    expect(cookieHeader).toContain("cookieA=fixture-a");
    expect(cookieHeader).toContain("cookieB=fixture-b");
  });

  it("retires a browser session only after its active cookie write drains", async () => {
    const browserPath = await copyFixtureToTemporaryFile("storage-state.json");
    const profilePath = await copyFixtureToTemporaryFile(
      "session-profile.json",
    );
    ...
    const provider = new StorageStateProvider(
      ...
      async (path, value) => {
        markStarted();
        await writeGate;
        await atomicWritePrivateJson(path, value);
      },
      new URL(origin),
    );
    await provider.load();
    const activeWrite = provider.applySetCookies(new URL(origin), [
      "cookieA=fixture-before-retire; Path=/; Secure",
    ]);
    await writeStarted;
    let retired = false;
    const retirement = provider.retire().then(() => {
      retired = true;
    });
    await Promise.resolve();
    expect(retired).toBe(false);
    releaseWrite();
    await Promise.all([activeWrite, retirement]);
    const afterDrain = await readFile(browserPath, "utf8");

    await provider.applySetCookies(new URL(origin), [
      "cookieB=fixture-after-retire; Path=/; Secure",
    ]);
    expect(await readFile(browserPath, "utf8")).toBe(afterDrain);
    const retiredCookies = await (
      await provider.load()
    ).jar.getCookieString(origin);
    expect(retiredCookies).toContain("cookieA=fixture-before-retire");
    expect(retiredCookies).not.toContain("cookieB=fixture-after-retire");
  });
  ...
  it.each([
    [
      ...
      "remove-me=value; Path=/; Max-Age=0",
      "remove-me",
    ],
    [
      "updates Path cookies",
      "api-cookie=updated; Path=/api; Secure",
      "api-cookie=updated",
    ],
    [
      "keeps domain and host-only cookies distinct",
      "domain-cookie=updated; Domain=fixture.invalid; Path=/; Secure",
      "domain-cookie=updated",
    ],
    ...
    const state = (await readValidStorageState(browserPath)) as {
      cookies: Array<{
        name: string;
        domain: string;
        path: string;
    ...
    if (expected === "remove-me") {
      expect(
        state.cookies.find((cookie) => cookie.name === expected),
      ).toBeUndefined();
    } else {
      expect(
        state.cookies.some(
          (cookie) => `${cookie.name}=${cookie.value}` === expected,
        ),
      ).toBe(true);
    }
    expect(
      state.cookies.filter((cookie) => cookie.name === "fixture_session"),
    ).toHaveLength(1);
  });
  ...
  it("serializes positive Max-Age as an absolute expiry and leaves missing SameSite unset", async () => {
    ...
    await provider.load();
    await provider.applySetCookies(new URL(origin), [
      "max-age-cookie=value; Max-Age=120; Path=/; Secure",
    ]);
    const state = (await readValidStorageState(browserPath)) as {
      cookies: Array<{ name: string; expires: number; sameSite?: string }>;
    };
  ...
  it("creates missing private-writer parent directories with restrictive permissions", async () => {
    ...
    expect(await readValidStorageState(target)).toEqual({
      originPin: "fixture-origin-pin",
    });
  ...
  it("restores both existing files and cleans artifacts when the second replacement fails", async () => {
    ...
    await expect(
      atomicWritePrivateJsonPair(
        ...
        async (from, to) => {
          if (to === profilePath && from.endsWith(".tmp"))
    ...
    expect(
      (await readdir(dirname(storagePath))).filter(
  ...
  it("retries the first backup cleanup failure without rolling back a committed pair", async () => {
    ...
    expect(await readValidStorageState(storagePath)).toEqual({
      changed: "storage",
    });
    expect(await readValidStorageState(profilePath)).toEqual({
      changed: "profile",
    });
    expect(
      (await readdir(dirname(storagePath))).filter(
